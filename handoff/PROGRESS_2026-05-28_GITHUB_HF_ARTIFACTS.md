...
<!-- FLUX2_LATENT_STD_DIAGNOSTIC_20260528 -->

## FLUX.2 latent_std diagnostic completed

更新时间：`2026-05-28T12:33Z`

Diagnostic script added locally:
...
```text
/workspace/PDM/handoff/FLUX2_LATENT_STD_DIAGNOSTIC_2026-05-28.md
```
...
- `latent_std ~= 1.714` is a real FLUX cache property, confirmed by two independent cache scans and consistent with the earlier 1024-image reconstruction diagnostic.
- This is not a FLUX VAE reconstruction failure. The reconstruction-ceiling diagnostic remains healthy.
- The current B3/MeanFlow FLUX issue is likely endpoint-scale mismatch: raw FLUX data endpoint std `~1.714`, but transport noise/sampler endpoint is `N(0,1)` and eval decode applies identity scale.
- First recommended validation: scalar smoke with `latent_multiplier=0.5834159221481118` and decode `pre_decode_scale=1.7140430386576422`, then per-channel normalization if that improves trend.
