#!/usr/bin/env python3
"""
PDM-3-HT Experiment 5: Mixed Precision Engineering / Synthetic PAE Latent JVP.

This experiment starts from Experiment 4 and uses official PAE repository
...
New mixed-precision engineering recipe:
    mode = bf16_backbone_fp32_jvp

    * JVP / finite-difference target path is forced to fp32, no autocast, no TF32.
      This is the correctness path and avoids the bf16 JVP FD drift observed in
      the Experiment-4 bf16_autocast diagnostic.
    * Training backbone path uses ordinary torch.autocast(..., bfloat16) forward
      for u only. The target v-du is computed under no_grad fp32 JVP and detached,
      so loss.backward() differentiates only through the bf16/autocast backbone
      forward and does not backpropagate through torch.func.jvp.
"""
...
def parse_args() -> argparse.Namespace:
    ...
    p.add_argument(
        ...
        type=str,
        default="fp32",
        help="comma-separated: fp32,bf16_autocast,bf16_backbone_fp32_jvp",
    )
    p.add_argument(
        "--latent-modes",
        type=str,
...
def make_config(args: argparse.Namespace) -> Config:
    ...
    allowed_modes = {"fp32", "bf16_autocast", "bf16_backbone_fp32_jvp"}
...
def backbone_forward_mode(mode: str) -> str:
    """Mode used by the trainable backbone u forward."""
...
def jvp_target_mode(mode: str) -> str:
    """Mode used by JVP/FD target correctness path."""
    if is_mixed_bf16_backbone_fp32_jvp(mode):
        return "fp32"
    return mode
...
def short_train_loop(
    ...
    completed = 0
    model.train()
    mixed_recipe = is_mixed_bf16_backbone_fp32_jvp(mode)
    u_mode = backbone_forward_mode(mode)
    du_mode = jvp_target_mode(mode)
    for step in range(1, cfg.train_steps + 1):
        ...
        assert isinstance(latent_meta, dict)
        r_eq_t_count = int((r == t).detach().sum().cpu())
        r_eq_t_total = int(r.numel())
        ...
        rec: Dict[str, Any] = {
            "step": step,
            "precision_recipe": mode,
            "backbone_forward_mode": u_mode,
            "jvp_target_mode": du_mode,
            "target_detached": True,
            "latent_meta": latent_meta,
            "r_eq_t_count": r_eq_t_count,
            "r_eq_t_total": r_eq_t_total,
            "r_eq_t_fraction_realized": r_eq_t_count / max(1, r_eq_t_total),
        }
        try:
            if mixed_recipe:
                # Correctness/target path: fp32 JVP under no_grad. The target is
                # explicitly detached; gradients must not flow through jvp().
                reset_peak_memory(device)
                cuda_sync(device)
                target_start = time.perf_counter()
                with torch.no_grad():
                    _u_jvp, du, _ = full_bundle_jvp(model, zt, r, t, v, du_mode, device)
                    target = (v - du).detach()
                cuda_sync(device)
                ...
                # Trainable path: ordinary bf16/autocast backbone forward for u.
                # loss.backward() differentiates through this path only.
                reset_peak_memory(device)
                cuda_sync(device)
                backbone_start = time.perf_counter()
                u = call_model(model, zt, r, t, u_mode, device)
                cuda_sync(device)
...
def run_one(cfg: Config, grid: int, latent_mode: str, mode: str, device: torch.device, pae_stats: PaeStats) -> Dict[str, Any]:
    ...
    seed += {"fp32": 0, "bf16_autocast": 101, "bf16_backbone_fp32_jvp": 211}[mode]
    ...
    record: Dict[str, Any] = {
        ...
        "precision_recipe": {
            "mode": mode,
            "backbone_forward_mode": backbone_forward_mode(mode),
            "jvp_target_mode": jvp_target_mode(mode),
            "fd_audit_mode": jvp_target_mode(mode),
    ...
    try:
        ...
        # Forward-only baseline. In the mixed recipe this is the trainable
        # bf16/autocast backbone path; fp32 reference forward is measured
        # separately without contaminating the bf16 forward peak.
        model.eval()
        reset_peak_memory(device)
        cuda_sync(device)
        start = time.perf_counter()
        with torch.no_grad():
            u_forward = call_model(model, zt, r, t, u_mode, device)
        cuda_sync(device)
        ...
        if is_mixed_bf16_backbone_fp32_jvp(mode):
            ...
            with torch.no_grad():
                u_forward_fp32_ref = call_model(model, zt, r, t, "fp32", device)
            cuda_sync(device)
        ...
        start = time.perf_counter()
        with torch.no_grad():
            u, du, tangents = full_bundle_jvp(model, zt, r, t, v, du_mode, device)
        cuda_sync(device)
        ...
        if cfg.run_fd and n <= cfg.fd_max_tokens:
            ...
            cuda_sync(device)
            start = time.perf_counter()
            du_fd = central_fd(model, (zt, r, t), tangents, cfg.fd_eps, du_mode, device)
            cuda_sync(device)
        ...
        with torch.no_grad():
            t_eq = t.clone()
            r_eq = t_eq.clone()
            _u_eq, du_eq, _ = full_bundle_jvp(model, zt, r_eq, t_eq, v, du_mode, device)
            target_eq = v - du_eq
...
def gate_summary(results: List[Dict[str, Any]]) -> Dict[str, Any]:
    ...
    correctness_modes = {"fp32", MIXED_BF16_BACKBONE_FP32_JVP_MODE}
    correctness = [r for r in ok if r.get("mode") in correctness_modes]
    correctness_1024 = [r for r in correctness if r.get("n_tokens") == 1024]
    fd_vals = [r["fd_rel_err_full_jvp"] for r in correctness_1024 if "fd_rel_err_full_jvp" in r]
    max_fd = max(fd_vals) if fd_vals else None
    max_degen = max((r.get("degenerate_target_minus_v_max_abs", float("inf")) for r in correctness_1024), default=None)
    train_all_finite = all(
        ...
        for r in correctness_1024
    ) if correctness_1024 else False
    jvp_all_finite = all(r.get("du_finite", False) and r.get("target_finite", False) and r.get("u_finite", False) for r in correctness_1024) if correctness_1024 else False
    no_nan = all(not r.get("has_nan_or_inf", True) for r in correctness_1024) if correctness_1024 else False
    strict = bool(
        correctness_1024
        and jvp_all_finite
    ...
    return {
        ...
        "n_ok": len(ok),
        "n_error": len(errors),
        "correctness_modes": sorted(correctness_modes),
        "correctness_result_count": len(correctness),
        "correctness_1024_result_count": len(correctness_1024),
        "max_correctness_1024_fd_rel_err": max_fd,
        "max_correctness_1024_degenerate_target_minus_v_max_abs": max_degen,
        "correctness_1024_train_all_finite": train_all_finite,
        "correctness_1024_full_jvp_all_finite": jvp_all_finite,
        "correctness_1024_no_nan_or_inf": no_nan,
        # Backward-compatible keys for older aggregate snippets.  In a mixed-only
        # run they intentionally mirror the correctness path rather than imply
        # that the run mode itself was pure fp32.
...
def write_summary(out_dir: Path, payload: Dict[str, Any]) -> None:
    ...
    lines: List[str] = []
    lines.append("# Experiment 5 — Mixed Precision Engineering / Synthetic PAE Latent JVP 结果\n\n")
    ...
    lines.append("- official modules：`LightningDiTBlock` / `Attention` / `RMSNorm` / `SwiGLU` / `FinalLayer` / `VisionRotaryEmbeddingFast`\n")
    lines.append("- per-token wrapper：官方 block 子模块保留，AdaLN conditioning 从 `[B,D]` 改为 `[B,N,D]` 并沿 `dim=-1` chunk。\n")
    ...
    lines.append("- mixed recipe：`bf16_backbone_fp32_jvp` = bf16/autocast backbone forward/backward + fp32 no-autocast JVP/FD target path；target detach，梯度只走 backbone `u`。\n")
    ...
    lines.append("\n## Gate 摘要\n\n")
    lines.append(f"- correctness modes：`{gate.get('correctness_modes')}`\n")
    lines.append(f"- N=1024 max FD relative error：`{gate['max_correctness_1024_fd_rel_err']}`\n")
    lines.append(f"- N=1024 max r=t target-v max abs：`{gate['max_correctness_1024_degenerate_target_minus_v_max_abs']}`\n")
    lines.append(f"- N=1024 full JVP finite：`{gate['correctness_1024_full_jvp_all_finite']}`\n")
    lines.append(f"- N=1024 short train finite：`{gate['correctness_1024_train_all_finite']}`\n")
    ...
    lines.append("\n## 核心指标表\n\n")
    lines.append("| grid | N | latent | mode | fwd mode | jvp mode | status | params | FD rel err | bf16-vs-fp32 fwd rel | du/v | JVP sec | FD sec | FWD sec | train steps | final loss | train peak MB | JVP peak MB | degen max | NaN/Inf |\n")
    lines.append("|---:|---:|---|---|---|---|---|---:|---:|---:|---:|---:|---:|---:|---:|---:|---:|---:|---:|---|\n")
    for r in results:
        train = r.get("short_train", {})
        lines.append(
            f"| {r.get('grid')} | {r.get('n_tokens')} | {r.get('latent_mode')} | {r.get('mode')} | "
            f"{r.get('forward_only_mode')} | {r.get('jvp_mode')} | {r.get('status')} | "
            f"{r.get('model_params')} | {fmt(r.get('fd_rel_err_full_jvp'))} | {fmt(r.get('bf16_forward_vs_fp32_rel_err'))} | {fmt(r.get('du_over_v_norm_ratio'))} | "
    ...
    lines.append("- `sdpa_kernel=math` 时仍调用官方 `F.scaled_dot_product_attention` 分支，但强制数学后端以便 forward AD/JVP 可审计；flash/mem-efficient 后端不进入主路径。\n")
    lines.append("- `bf16_autocast` 直接包住 JVP 仍仅为反例诊断；本实验的可用 mixed recipe 是 **bf16 backbone + fp32 JVP target path 分离**。\n")
    lines.append("- 因 `target=(v-du).detach()`，mixed 训练步不对 JVP path 求反传；这是避免 Experiment-4 bf16 JVP backward dtype mismatch 的工程约束。\n")
    lines.append("- 该 stress 不给出 FID/FDr 或生成质量结论，也不证明 XL/1 全规模长训稳定性。\n")
    (out_dir / "mixed_precision_synthetic_jvp_summary.md").write_text("".join(lines), encoding="utf-8")
    # Keep the old filename too so existing collection scripts can still find a summary.
...
def main() -> None:
    ...
    print(f"[write] {out_dir / 'mixed_precision_synthetic_jvp_summary.md'}", flush=True)
