...
#   checkpoint keep: sparse HF archives every 100k steps + final 1.07M, keep current latest locally
...
ARCHIVE_EVERY=${B3_HF_ARCHIVE_EVERY:-100000}
CHECKPOINT_START_STEP=${B3_HF_CHECKPOINT_START_STEP:-100000}
ARCHIVE_EXTRA_STEPS=${B3_HF_ARCHIVE_EXTRA_STEPS:-1070000}
...
exec >> "$LOG" 2>&1
echo "[$(date -u +%Y-%m-%dT%H:%M:%SZ)] launcher_start exp=$EXP res=$RES repo=$REPO_ID remote_prefix=$REMOTE_PREFIX archive_every=$ARCHIVE_EVERY checkpoint_start=$CHECKPOINT_START_STEP archive_extra_steps=$ARCHIVE_EXTRA_STEPS eval_start=$EVAL_START_STEP stop_step=$STOP_STEP interval=$INTERVAL"
...
python3 "$EXP/scripts/watch_b3_hf_upload_and_cleanup.py" \
  ...
  --archive-extra-steps $ARCHIVE_EXTRA_STEPS \
