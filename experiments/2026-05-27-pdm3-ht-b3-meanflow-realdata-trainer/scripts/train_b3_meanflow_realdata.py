...
def parse_metric_json_from_text(text: str) -> Dict[str, Any]:
    """Parse last JSON metric object from an external eval command.

    The FLUX image-space wrapper now returns a JSON line containing ``fid`` and
    optional MMD/KID fields.  Keep this permissive so plain-text FID tools still
    work via ``parse_fid_from_text``.
    """
    found: Dict[str, Any] = {}
    for line in text.splitlines():
        line = line.strip()
        ...
        try:
            ...
            continue
        if isinstance(obj, dict) and any(k in obj for k in ("fid", "FID", "mmd2", "kid", "kid_x1000")):
            found = obj
    return found
...
def run_fid_command(
    *,
    ...
    metric_json = parse_metric_json_from_text(text)
    fid = None
    for key in ("fid", "FID", "fid50k", "FID50K"):
        if key in metric_json and metric_json[key] is not None:
            try:
                fid = float(metric_json[key])
            except Exception:
                fid = None
            break
    if fid is None:
    ...
    for key in ("mmd2", "kid", "kid_x1000", "ref_features_used", "num_images", "image_eval_status"):
        if key in metric_json:
            rec[key] = metric_json[key]
    if metric_json:
        rec["fid_metric_json"] = metric_json
    return rec
...
def run_eval(
    *,
    ...
    sample_batch_size = int(eval_cfg.get("sample_batch_size", num_samples))
    if sample_batch_size <= 0:
        raise ValueError(f"eval.sample_batch_size must be positive, got {sample_batch_size}")
    sample_batch_size = min(sample_batch_size, num_samples)
    sample_log_every_batches = int(eval_cfg.get("sample_log_every_batches", 0) or 0)
    ...
    record: Dict[str, Any] = {
        ...
        "num_sample_batches": int((num_samples + sample_batch_size - 1) // sample_batch_size),
    ...
    try:
        ...
        total_batches = int((num_samples + sample_batch_size - 1) // sample_batch_size)
        for batch_idx, s in enumerate(range(0, num_samples, sample_batch_size), start=1):
            e = min(num_samples, s + sample_batch_size)
            chunk = sample_meanflow_latents(
                model=eval_model,
                transport=transport,
                labels=labels[s:e],
            ...
            if sample_log_every_batches > 0 and (
                batch_idx == 1 or batch_idx == total_batches or batch_idx % sample_log_every_batches == 0
            ):
                cuda_sync(device)
                logger.info(
                    "eval sample progress step=%d batch=%d/%d count=%d/%d peakMB=%s",
                    step,
                    batch_idx,
                    total_batches,
                    e,
        ...
        samples = torch.cat(sample_chunks, dim=0) if len(sample_chunks) > 1 else sample_chunks[0]
        cuda_sync(device)
