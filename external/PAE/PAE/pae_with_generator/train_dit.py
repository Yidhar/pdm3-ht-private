...
def separate_weight_decay(modules, default_decay):
    ...
    if isinstance(modules, torch.nn.Module):
        params = modules.named_parameters()
    elif isinstance(modules, (list, torch.nn.ModuleList)):
        params = []
        for m in modules:
            if isinstance(m, torch.nn.Module):
                params.extend(list(m.named_parameters()))
    else:
        params = modules
...
def do_train(train_config, accelerator):
    ...
    ema_decay = train_config['train'].get('ema_decay', 0.9999)
    ...
    transport = create_transport(
        ...
        lognorm_mu = train_config['transport'].get('lognorm_mu', 0.0),
        lognorm_sigma = train_config['transport'].get('lognorm_sigma', 1.0),
        train_timestep_shift = train_config['transport'].get('train_timestep_shift', 1.0),
    )  # default: velocity; 
    if accelerator.is_main_process:
    ...
    wd = train_config['optimizer'].get('weight_decay', 0.0)
    optim_groups = separate_weight_decay(model, wd)
    if accelerator.is_main_process:
        logger.info(f"Weight decay analysis: "
                    f"{len(optim_groups[0]['params'])} params with decay, "
                    f"{len(optim_groups[1]['params'])} params without decay")
    ...
    while True:
        for x, y in loader:
            ...
            if 'max_grad_norm' in train_config['optimizer']:
                if accelerator.sync_gradients:
                    ...
                    grad_norm = accelerator.clip_grad_norm_(
                        model.parameters(), 
                        train_config['optimizer']['max_grad_norm']
                    )
            ...
            if train_config['optimizer'].get('use_lr_scheduler', False):
                steps_per_epoch = len(loader)
                warmup_steps = train_config['optimizer'].get('warmup_epochs', 40) * steps_per_epoch
                max_steps = train_config['train']['max_steps']
                

                base_lr = train_config['optimizer']['lr']

                end_lr = train_config['optimizer'].get('end_lr', base_lr) 

                if train_steps < warmup_steps:
                    curr_lr = base_lr
                else:
 
                    decay_range = max(1, max_steps - warmup_steps)
                    progress = min((train_steps - warmup_steps) / decay_range, 1.0)
                    curr_lr = base_lr + (end_lr - base_lr) * progress
            ...
            if train_steps % train_config['train']['log_every'] == 0:
                ...
                if accelerator.is_main_process:
                    ...
                    writer.add_scalar('GradNorm/train', avg_grad_norm, train_steps) #
    ...
    # Cleanup: ensure all processes sync before exit
    if accelerator.is_main_process:
        writer.close()
        logger.info("Done!")
...
def main():
    ...
    if accelerator.process_index == 0:
        print(f"==========================================")
        print(f"Switching to pae (Offline Latents)")
        print(f"PAE Config: {train_config['vae']}")
