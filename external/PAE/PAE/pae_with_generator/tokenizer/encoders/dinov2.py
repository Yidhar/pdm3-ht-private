...
class Dinov2withNorm(nn.Module):
    ...
    def dinov2_forward(self, x: torch.Tensor) -> torch.Tensor:
        ...
        # drop cls token and reg tokens
        cls_tokens = [hidden_states[:, 0] for hidden_states in x.hidden_states[1:]]
        encoder_hidden_states = [hidden_states[:, unused_token_num:] for hidden_states in x.hidden_states[1:]]
        
        return image_features, (cls_tokens, encoder_hidden_states)
