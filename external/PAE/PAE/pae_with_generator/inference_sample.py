...
# sample function
def do_sample(train_config, accelerator, ckpt_path=None, cfg_scale=None, model=None, vae=None, demo_sample_mode=False, batch_demo_mode=False, use_ema=True):
    """
    Run sampling.
    """
    ...
    if demo_sample_mode or batch_demo_mode:
        cfg_interval_start = 0.0
        timestep_shift = 0.9
        cfg_scale = 9.0
        train_config['sample']['num_sampling_steps'] = 250
        demo_class_list = [975, 3, 207, 387, 388, 88, 979, 279]
        demo_batch_size = 30
        print('Demo step:', train_config['sample']['num_sampling_steps'])

    base_sample_dir = './output/samples'
    sample_folder_dir = os.path.join(base_sample_dir, train_config['train']['exp_name'], folder_name)
    ...
    if use_ema:
        if "ema" in checkpoint:
            ...
            if accelerator.process_index == 0:
                print_with_prefix("Warning: EMA weights not found in checkpoint, falling back to normal weights.")
    else:
        # 加载标准权重
        checkpoint = checkpoint.get("model", checkpoint)
        if accelerator.process_index == 0:
            print_with_prefix("Using standard weights.")
    ...
    if demo_sample_mode:
        if accelerator.process_index == 0:
            images = []
            for label in tqdm(demo_class_list, desc="Generating Demo Samples"):
                ...
                with torch.autocast(device_type='cuda', dtype=torch.bfloat16):
                    samples = vae.decode(samples.to(torch.bfloat16))
            ...
            demo_name = train_config['train']['exp_name']
            Image.fromarray(grid).save(f'demo_images/{demo_name}.png')

        return None
    elif batch_demo_mode:
        # Distribute (class, sample_idx) tasks across all ranks
        demo_name = train_config['train']['exp_name']
        batch_demo_dir = os.path.join('demo_images', f'{demo_name}_batch')

        # Build flat list of all tasks: (label, sample_idx)
        all_tasks = [(label, sid) for label in demo_class_list for sid in range(demo_batch_size)]
        total_tasks = len(all_tasks)
        num_procs = accelerator.num_processes

        # Rank 0 creates all directories first
        if rank == 0:
            os.makedirs(batch_demo_dir, exist_ok=True)
            for label in demo_class_list:
                os.makedirs(os.path.join(batch_demo_dir, f'class_{label}'), exist_ok=True)
        accelerator.wait_for_everyone()

        # Each rank takes its slice of tasks
        tasks_per_rank = math.ceil(total_tasks / num_procs)
        start = rank * tasks_per_rank
        end = min(start + tasks_per_rank, total_tasks)
        my_tasks = all_tasks[start:end]

        desc = f"Rank {rank} Batch Demo ({len(my_tasks)} tasks)"
        for label, sample_idx in tqdm(my_tasks, desc=desc, disable=(rank != 0)):
            z = torch.randn(1, model.in_channels, latent_size, latent_size, device=device)
            y = torch.tensor([label], device=device)
            z = torch.cat([z, z], 0)
            y_null = torch.tensor([1000] * 1, device=device)
            ...
            with torch.autocast(device_type='cuda', dtype=torch.bfloat16):
                samples = vae.decode(samples.to(torch.bfloat16))
            samples = torch.clamp(255.0 * samples, 0, 255).permute(0, 2, 3, 1).to("cpu", dtype=torch.uint8).numpy()
            class_dir = os.path.join(batch_demo_dir, f'class_{label}')
            Image.fromarray(samples[0]).save(os.path.join(class_dir, f'{sample_idx:04d}.png'))

        accelerator.wait_for_everyone()
        if rank == 0:
            print(f"Batch demo images saved to {batch_demo_dir}")
        return None
    else:
        ...
        assert fid_num % num_classes == 0, f"fid_num ({fid_num}) must divide by num_classes ({num_classes})"
        samples_per_class = fid_num // num_classes
        full_labels = torch.arange(num_classes).repeat_interleave(samples_per_class)

        if total_samples > fid_num:
            extra_needed = total_samples - fid_num
            extra_labels = torch.randint(0, num_classes, (extra_needed,))
            full_labels = torch.cat([full_labels, extra_labels])

        start_idx = rank * samples_needed_this_gpu
        this_gpu_labels = full_labels[start_idx : start_idx + samples_needed_this_gpu]
        

        for batch_idx in pbar: 
            batch_start = batch_idx * n
            batch_end = batch_start + n
            

            y_raw = this_gpu_labels[batch_start:batch_end].to(device)
            ...
            samples = torch.clamp(samples.mul(255.0), 0, 255).permute(0, 2, 3, 1).to("cpu", dtype=torch.uint8).numpy() # PAE no need pixel denorm
 
            for sample_in_batch_idx, sample in enumerate(samples):
                global_idx = start_idx + batch_start + sample_in_batch_idx
                Image.fromarray(sample).save(f"{sample_folder_dir}/{global_idx:06d}.png")
